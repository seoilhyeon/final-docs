...
- `settlement_pending_amount`와 `settlement_failed_amount`는 wallet/API projection field이며 DB/account column이 아니다. `settlement_pending_balance`/`settlement_failed_balance` 컬럼은 두지 않는다. pending은 사용자에게 보여줄 환급 예정액으로, 정산 row가 있으면 미지급 `settlement_item.refund_amount` 중 `PENDING`/`RUNNING`/`RETRY_WAIT` 상태를 사용하고 정산 row가 없는 `CLOSED` 크루는 최신 `FINALIZED`/`SUCCEEDED` 일일 정산 스냅샷의 `expected_refund_amount`를 사용한다. failed는 `FAILED` 상태 미지급 `settlement_item.refund_amount`를 분리해 보여준다.
...
- `active_locked_amount`는 `locked_balance`를 source로 설명하는 projection-only field다. `settlement_pending_amount`/`settlement_failed_amount`는 `settlement_item.refund_amount`, `settlement.status`, `point_history_id` linkage와 pre-settlement finalized daily snapshot을 source로 하는 환급 projection이므로 `locked_balance == active_locked_amount + settlement_pending_amount` reconciliation check를 두지 않는다.
