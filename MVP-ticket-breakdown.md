...
**What:**  
크루 입장, 잔액 차감, 보증금 잠금, 중복 입장 방지, 인원 초과 방지 로직을 구현한다. 인원 미달 시 환급 가능한 상태 정보도 남긴다.
...
**What:**  
크루 입장 버튼, 예치 확인, 잔액 부족 안내, 충전 화면 이동, 이미 참여한 상태 표시 UI를 구현한다.
...
DB-level upsert(`ON CONFLICT`)를 사용해 동시 요청에서도 오류 없이 일관된 상태로 수렴해야 한다.
