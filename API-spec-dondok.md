...
- `activity_stats.*`는 `Settlement.status = SUCCEEDED` + `SettlementItem`만 사용한다. raw `mission_log.SUCCESS`나 dashboard projection을 사용하지 않는다.
...
- `activity_stats.average_success_rate`는 settlement-time cadence context(`settlement.rule_context_snapshot` + settlement item period)로 계산한다. 해당 context가 안전하게 해석되지 않으면 `null`을 반환하고, 현재 `mission_rule`/`mission_schedule_day`로 과거 정산을 재해석하지 않는다.
...
- 내부 `member.id`, participant 내부 ID, 내부 FK, `reject_memo`, raw moderation chain, settlement item 내부 ID는 노출하지 않는다.
...
| 승인 (`approve`) | 기존 reserve를 `LOCKED`로 확정 (`reserved_balance → locked_balance` bucket transition 수행, `CREW_DEPOSIT_LOCK` point_history row를 생성/재사용) |
...
| `settlement_pending_amount` | 사용자에게 보여줄 환급 예정액. 정산 row가 있으면 `point_history_id IS NULL`이고 settlement 상태가 `PENDING`/`RUNNING`/`RETRY_WAIT`인 `settlement_item.refund_amount` 합계, 정산 row가 아직 없고 크루가 `CLOSED`이면 최신 `FINALIZED`/`SUCCEEDED` 일일 정산 스냅샷의 `expected_refund_amount` 합계 |
...
- `settlement_pending_amount`는 locked principal split이 아니라 환급 예정 projection이다. 정산 row가 생성된 크루는 settlement item을 우선하고, 정산 row가 없는 `CLOSED` 크루는 최신 `FINALIZED`/`SUCCEEDED` 일일 정산 스냅샷을 fallback으로 사용한다. `active_locked_amount + settlement_pending_amount = locked_balance` 불변식을 두지 않는다. 불일치/이상값은 `settlement_item.refund_amount`, `settlement_item.point_history_id`, `settlement.status`, `daily_settlement_participant_snapshot.expected_refund_amount`, `point_history`, `crew_participant`, `point_account`를 함께 대조한다.
...
**`reference_meta` 필드 매핑**

- `reference_meta`는 도딘 내역 UI 표시용 read-model이며 persisted `point_history` column이 아니다.
- `reference_type = CREW_PARTICIPANT`이면 `point_history.reference_id -> crew_participant.id -> crew`로 조회해 `{ "crew_id", "crew_title" }`을 내려준다.
- `reference_type = SETTLEMENT_ITEM`이면 `point_history.reference_id -> settlement_item.id -> settlement -> crew`로 조회해 `{ "crew_id", "crew_title" }`을 내려준다.
- `reference_type = POINT_CHARGE`처럼 크루 맥락이 없는 이력은 `reference_meta = null`이다.
- `reference_meta`에는 사용자에게 보여줄 최소 표시 정보만 포함한다. 내부 `crew_participant.id`, `settlement_item.id`, member 내부 ID, 정산 계산 상세는 포함하지 않는다.
...
- `CREW_DEPOSIT_RESERVE`는 `PENDING` 신청 reserve 이벤트다. 일반 참여자의 `PENDING` 신청은 `available_balance -= deposit_amount` / `reserved_balance += deposit_amount`를 반영한다. host auto-created `LOCKED`는 `available_balance -= deposit_amount` / `locked_balance += deposit_amount`로 반영한다.
- `CREW_DEPOSIT_LOCK`는 `PENDING` 승인 확정 이벤트다 (`reserved_balance → locked_balance`). 승인 경로는 `CREW_DEPOSIT_LOCK` `point_history` row를 append/reuse한다.
