...
| B-02 | P0                              | Crew           | 크루 생성과 규칙 설정             | 주최자가 공개/비공개 크루와 미션 규칙을 정의할 수 있다.                                      | B-01                       | US-02         | M    |
...
| B-06 | P0                              | Wallet / Crew  | 크루 입장과 보증금 예치           | 참여 시점에 보증금을 잠그고 정산 전까지 보관한다.                                            | B-02, B-03 또는 B-04, B-05 | US-06         | M    |
...
| B-10 | P0                              | Settlement     | 정산 배치와 환급 처리             | 종료 후 공정한 계산으로 자동 환급을 끝낸다.                                                  | B-06, B-08, B-09           | US-10         | L    |
...
| Phase 2. 참여와 인증 | B-03, B-04, B-06, B-07, B-08, B-17, B-18 | 사용자가 실제 챌린지에 들어가 인증하고 피드/리액션으로 확인한다. | 공개/비공개 경로 모두에서 보증금 예치 후 인증 성공/실패가 구분되고, feed-eligible 인증 로그의 피드 노출과 리액션 멱등성이 검증되어야 한다. |
...
**Conversation:**  
이 story는 크루의 계약서를 만드는 역할을 한다. 기간은 `1주~3개월`, 인원은 `최대 10명`, 보증금은 `1,000원~100만원`이며 `1,000원 단위`만 허용한다. 비공개 크루는 `6자리 참여 코드`가 필요하다. 주최자는 생성 직후 운영 권한을 가진다.
...
**Conversation:**  
입장과 동시에 보증금은 사용 가능 잔액에서 빠지고 잠금 상태가 된다. 이 잠금 금액은 정산 전까지 일반 사용 가능 포인트와 분리되어야 한다. 인원 미달로 크루가 시작되지 못하면 예치금은 전액 환급되어야 한다.
...
5. 크루가 시작 조건을 만족하지 못해 무산되면 예치된 보증금은 전액 환급된다.
6. 사용자는 내가 참여 중인 크루와 예치된 금액을 확인할 수 있다.
...
### US-10. 미션 종료 후 자동 정산과 환급

**Priority:** P0  
**Card:** 미션이 끝나면 시스템은 자동으로 지분율을 계산해 환급을 끝내야 한다.  
**Design:** 미정. 기준 문서: [PRD-god-saving.md](./PRD-god-saving.md)

**Conversation:**  
정산은 MVP의 가장 위험한 구간이다. 배치는 `종료 익일 새벽`에 돌아야 하고, 계산은 `BigDecimal`, `원 단위 절사`, `잔액 처리 규칙`을 따라야 한다. `전체 성공 횟수 0`, `절사 후 잔액`, `1위 동점`, `중도 탈퇴` 같은 예외를 다뤄야 한다.

**Confirmation:**

1. 미션 종료 후 시스템은 `익일 새벽`에 정산 배치를 실행한다.
2. 정산은 사용자별 성공 횟수와 크루 전체 성공 횟수를 기준으로 지분율을 계산한다.
3. 계산은 `BigDecimal` 기반으로 처리되고 금액은 `원 단위`로 절사된다.
4. 크루 전체 성공 횟수가 `0`이면 전원 균등 환급한다.
5. 절사 후 남은 잔액은 기여도 1위에게 지급하고, 동점이면 `성공 횟수 우선 -> 그래도 같으면 랜덤` 규칙을 따른다.
6. 배치 실패 시 최대 `3회` 재시도하고, 계속 실패하면 어드민 알림 대상으로 남긴다.
...
1. 이 문서를 기준으로 `Epic -> Story -> Task` 3단 분해를 해서 개발 티켓 단위로 내린다.
2. `US-10` 정산 규칙은 별도 설계 문서로 예제 데이터와 함께 명세를 고정한다.
3. `US-07`~`US-10` 구간은 API 명세와 테스트 케이스를 먼저 쓰고 구현에 들어간다.
