...
- [API-spec-dondok.md](./API-spec-dondok.md)
...
| T-04A  | 2     | BE    | US-06A        | 시스템 자동 activation job과 lifecycle 전이 구현 | T-03, T-09  |
| T-04B  | 2     | FE    | US-06A        | 자동 시작 예정 상태 안내 UI 구현                | T-04A       |
...
| T-20   | 3     | FE    | US-09         | 현재 기준 지분율 대시보드 UI 구현               | T-16        |
...
**What:**
회원가입, 로그인, JWT 발급/검증, 보호 API 접근 제어를 구현한다. Identity persistence는 `ERD-dondok.md`의 `member` 정의를 따르고, auth subject contract는 `API-spec-dondok.md`의 인증 규칙을 따른다. `US-01A`를 위해 `GET /api/me` profile read와 `PATCH /api/me/profile` profile update도 같은 Auth / Profile 범위에서 구현한다. 민감한 인증 실패 응답은 과도한 내부 정보를 노출하지 않도록 정리한다.
...
**What:**
공개/비공개 크루 생성 API와 규칙 검증을 구현한다. 기간 `1주~3개월`, 최대 인원 `15명`, 보증금 `1,000원~100,000원`, `1,000원 단위`, 비공개 `6자리 참여 코드`, `min_participants`, `recruitment_deadline`, 예정 시작이자 시스템 자동 activation anchor인 `start_at` 생성 규칙을 포함한다.
...
- `min_participants`는 host command precondition이 아니라 `start_at` 시스템 자동 activation eligibility condition으로 문서화된다.
...
### T-04A. 시스템 자동 activation job과 lifecycle 전이 구현

**Why:**
최소 인원 충족만으로 즉시 ACTIVE 전이하면 정산 기준 시각, 인증 가능 시점, participant baseline이 흔들린다. MVP에서는 `start_at` 시스템 lifecycle 평가를 단일 activation commit point로 고정해야 한다.

**What:**
`start_at` 시스템 자동 activation job을 구현한다. `RECRUITING` 상태, `JOINED` participant baseline, 예치 Lock 완료, `min_participants` 충족 여부, `activated_at = start_at` 기록, `ACTIVE` 전이를 하나의 조건부 transaction으로 처리한다. Brownfield `POST /api/rooms/{roomId}/start`는 MVP active contract가 아니다.

**Acceptance Criteria:**

- Host는 미션 시작 authority가 아니며 activation을 직접 요청할 수 없다.
- 성공 시 `status = ACTIVE`와 `activated_at = start_at`이 함께 기록된다.
...
- `start_at` 평가 시점에 조건을 만족하지 못하면 자동 activation은 실패/취소 경로로 분기한다.
- 동시 activation job 실행은 하나의 조건부 전이만 성공한다.

### T-04B. 자동 시작 예정 상태 안내 UI 구현

**Why:**
`start_at` 기반 시스템 자동 activation 정책을 사용자가 이해하려면 상세 화면에서 자동 시작 예정 시각, 시작 가능 조건, 실패/취소 상태를 명확히 보여줘야 한다.

**What:**
시스템 자동 시작 예정 안내, 최소 인원 충족 여부, `recruitment_deadline`, `start_at` activation anchor 안내, 자동 시작 실패/성공 상태 메시지를 구현한다.

**Acceptance Criteria:**

- Host와 참여자는 `RECRUITING` 방에서 자동 시작 예정 시각과 충족/미충족 조건을 볼 수 있다.
- 수동 시작 버튼은 MVP active contract로 노출하지 않는다.
...
**What:**
크루 입장, 잔액 차감, 보증금 잠금, 중복 입장 방지, 인원 초과 방지 로직을 구현한다. Withdrawal API/상태는 brownfield/deferred visibility로만 남기며, MVP active flow에서는 즉시 환급이나 frozen settlement baseline 변경 권한으로 구현하지 않는다.
...
- MVP public contract에서는 `POST /api/rooms/{roomId}/withdraw`를 active user flow로 제공하지 않거나 `WITHDRAW_NOT_ALLOWED`로 방어한다.
- `WITHDRAWN`/`withdrawn_at`은 brownfield/deferred reference로만 남기며, 잠긴 보증금 즉시 환급·final settlement mutation·point ledger 직접 변경으로 해석하지 않는다.
- `AFTER_WITHDRAWN`/`AFTER_WITHDRAWN_AT`은 deferred guard vocabulary이며 MVP frozen `JOINED` baseline을 소급 변경하지 않는다.
...
- MVP에서는 탈퇴 확인 UI를 active flow로 노출하지 않는다. 노출이 필요한 경우에도 brownfield/deferred 상태로 안내하고 `WITHDRAW_NOT_ALLOWED` 또는 동등한 방어 흐름을 사용한다.
- 탈퇴 관련 문구는 보증금 즉시 환급, final settlement mutation, point ledger 직접 변경처럼 표시하지 않는다.
...
**What:**
`MissionLog`와 frozen `JOINED` participant baseline을 기준으로 사용자 raw 성공 횟수, 추정 인정 성공 횟수, 크루 전체 추정 인정 성공 합계, 현재 기준 예상 환급금, 보증금 대비 예상 차이, 추정 수행/참여도 표시 순서를 반환하는 Dashboard API 조회 모델을 구현한다. 이 조회 모델은 query-time deterministic current-basis projection이며, `MissionLog.server_time`과 `Asia/Seoul` 기준으로 API spec의 representative rule을 적용한다. `point_history`를 projection source로 사용하지 않고, snapshot/cache/aggregate table이나 Redis ranking/cache에 projection 값을 source of truth처럼 저장하지 않는다. 전체 추정 인정 성공 횟수 `0`인 경우도 equal-principal refund estimate 또는 projection input 부족 notice로 처리한다.
...
- 예상 환급금은 정산 remainder/winner/draw를 포함하지 않는 deterministic current-basis estimate로 계산된다.
- 전체 추정 인정 성공 횟수 `0`인 경우에도 오류 없이 결과를 반환한다.
- `CLOSED` + not `SUCCEEDED` 상태는 저장된 snapshot이 아니라 `room.end_at` cutoff 기반 `FROZEN` projection으로 query-time 계산된다.
...
**What:**
지분율 계산, `BigDecimal` 처리, `원 단위 절사`, deterministic remainder allocation, `전체 성공 횟수 0` all-fail equal-principal refund, `중도 탈퇴` brownfield/deferred guard를 구현한다.

**Acceptance Criteria:**

- 지분율과 환급액 계산이 독립 함수 또는 서비스로 동작한다.
- 전체 성공 횟수 `0` 규칙이 정확히 적용된다.
- 절사 후 잔액은 deterministic remainder allocation으로 처리되고, 동점은 payout이 아닌 표시/설명 ordering으로만 반영된다.
...
**What:**
Settlement-design의 final settlement product anchor 이후 일반 정산 배치, `start_at` 자동 activation 실패 방의 `RECRUITING -> CANCELLED` 취소 batch, 실패 `3회` 재시도, 동일 미션/취소 정산 멱등 처리, 어드민 확인 대상 상태 저장을 구현한다. 필요 시 락/큐/상태 전이를 포함한다.

**Acceptance Criteria:**

- 종료된 미션에 대해 정산 배치가 자동 실행된다.
- `start_at`까지 시작되지 않은 `RECRUITING` 방은 `CANCELLED` 처리되고 취소형 정산이 생성된다.
- 자동 activation과 취소 batch가 경합해도 하나의 조건부 전이만 성공한다.
...
- 인원 미달 취소 또는 all-fail equal-principal refund 결과도 같은 구조로 조회된다.

### T-20. 현재 기준 지분율 대시보드 UI 구현
...
**What:**
`GET /api/rooms/{roomId}/dashboard`를 소비해 내 성공 횟수, 추정 인정 성공 횟수, 전체 추정 인정 성공 횟수, 현재 기준 추정 지분율, 현재 기준 예상 환급금, 보증금 대비 예상 차이, 추정 수행/참여도 표시 순서를 보여주는 대시보드 UI를 구현한다. 전체 추정 인정 성공 `0` 상태도 설명 문구와 함께 처리한다. FE는 Dashboard 값을 최종 정산값처럼 자체 계산하거나, client-side settlement/remainder/winner/draw 계산을 수행하지 않는다.

**Acceptance Criteria:**

- 사용자는 내 raw 성공 횟수와 추정 인정 성공 횟수를 구분해 볼 수 있다.
- 현재 기준 추정 지분율, 예상 환급금, 보증금 대비 예상 차이가 “예상”, “현재 기준”, “추정” 라벨로 표시된다.
- 예상 환급금은 최종 환급금이 아니라 base estimate이며 최종 정산 시 1원 단위 차이가 날 수 있음을 오해 없이 안내한다.
- 전체 추정 인정 성공 횟수 `0`인 경우에도 UI가 정상 동작한다.
- `rank_estimated`는 예상 수익/환급금 순위가 아니라 추정 수행/참여도 표시 순서로 표시된다.
...
**What:**
정산 예시 데이터와 기대 환급 결과를 만드는 테스트 세트를 작성한다. `전체 성공 횟수 0 equal-principal refund`, `잔액 발생`, `동점 표시 ordering`, `중도 탈퇴 brownfield/deferred`, `인원 미달`, `start_at` 자동 activation 실패 취소, 자동 activation/취소 batch 경합 케이스를 포함한다. Dashboard projection 회귀 fixture는 최종 정산 골든 데이터와 구분해, query-time current-basis projection과 final settlement의 차이를 함께 검증한다.
...
- Dashboard projection fixture가 `rank_estimated` 동률 시 `participant_id ASC`, zero-total equal-principal refund estimate, base estimate와 final settlement deterministic remainder allocation 차이를 포함한다.
...
**What:**
인증 결과, 지분 변화 등 알림 이벤트 스키마와 SSE 발행 엔드포인트, 사용자별 구독 인증을 구현한다. SSE 계약은 `API-spec-dondok.md`의 알림/SSE 섹션과 `ERD-dondok.md`의 member identity invariant를 따른다.

**Acceptance Criteria:**

- 알림/SSE endpoint, auth, payload, delivery semantics가 `API-spec-dondok.md`의 알림/SSE 계약을 따른다.
- identity 사용이 `ERD-dondok.md`의 member invariant와 API Spec의 auth contract를 따른다.
...
- 구현은 `API-spec-dondok.md`의 알림/SSE 계약과 ADR의 notification durability 제외 범위를 벗어나지 않는다.
