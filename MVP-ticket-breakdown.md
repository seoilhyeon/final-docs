...
**What:**  
인증 결과, 지분 변화 등 알림 이벤트 스키마와 SSE 발행 엔드포인트, 사용자별 구독 인증을 구현한다. MVP 내부 routing key는 email을 사용할 수 있지만, email은 canonical identity가 아니며 `memberId`/UUID가 canonical identity라는 정책을 따른다.
