...
| --- | --- | --- | --- |
| 1 | 크루 가입 신청 | MVP required | Host action hint. `PENDING` reserve state is canonical crew/application API; push is not capacity or deposit authority. |
| 2 | 크루 가입 신청 철회 | MVP required | Cancellation notification is hint only; refetch terminal `CANCELLED` state and reserve refund ledger. |
| 3 | 가입 승인 완료 | MVP required | Approval hint; refetch canonical `LOCKED` participant state. Approval push is not deposit/settlement authority. |
| 4 | 가입 거절 | MVP required | Rejection hint; refetch terminal `REJECTED` state and reserve refund ledger. Avoid person-level judgment wording. |
...
| 15 | 예상 환급금 변동 | MVP candidate / rewrite required | Current-basis expected refund/progress summary only. Avoid failure-profit, “상승/추가 수익”, relative-failure dopamine, guaranteed payout, or final amount framing. |
